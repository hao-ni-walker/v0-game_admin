 ...
 * 用户筛选组件
 */
export function FilterBar({ onSearch, loading = false }: FilterBarProps) {
  const {
    filters,
    updateFilter,
    resetFilters,
  ...
  // 本地表单状态（用于高级筛选弹窗）
  const [formData, setFormData] = useState<Partial<AdminUserFilters>>({});
   ...
   * 更新表单字段值
   */
  const updateFormField = (key: keyof AdminUserFilters, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  const handleSearch = () => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      updateFilter(key as keyof AdminUserFilters, value);
    });
    applyFilters();
    onSearch?.();
  };
        ...
        >
          ...
          {hasActiveFilters && (
            <span className='ml-2 rounded-full bg-primary px-2 py-0.5 text-xs text-primary-foreground'>
              {Object.keys(filters).filter((k) => filters[k as keyof AdminUserFilters] !== undefined && filters[k as keyof AdminUserFilters] !== '').length}
            </span>
          )}
        </Button>
        {hasActiveFilters && (
