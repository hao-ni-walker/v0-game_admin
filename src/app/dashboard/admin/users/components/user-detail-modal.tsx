...
import { AdminUserDetail, PeriodType } from '../types';
import {
  formatCurrency,
  formatDateTime,
  getUserStatusText,
...
interface UserDetailModalProps {
  ...
  onAdjustWallet: (user: AdminUserDetail) => void;
...
export function UserDetailModal({
  ...
  const loadUserDetail = async () => {
    ...
    setLoading(true);
    try {
      const userData = await onRefresh(userId);
      if (userData) {
        setUser(userData);
      }
    } finally {
  ...
  const filteredSpinQuotas = React.useMemo(() => {
    if (!user?.spin_quotas) return [];
    if (spinQuotaFilter === 'all') return user.spin_quotas;
    return user.spin_quotas.filter((q) => q.period_type === spinQuotaFilter);
  }, [user?.spin_quotas, spinQuotaFilter]);
              ...
              {user.wallet ? (
                <>
                  <div className='flex justify-end'>
                    <Button onClick={() => onAdjustWallet(user)}>
