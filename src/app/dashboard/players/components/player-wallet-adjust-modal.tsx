...
import { PlayerDetail, WalletAdjustFormData, WalletField, AdjustType } from '../types';
import { formatCurrency } from '../utils';

interface PlayerWalletAdjustModalProps {
  open: boolean;
 ...
 * 钱包调整弹窗组件
 */
export function PlayerWalletAdjustModal({
  open,
  player,
  onClose,
  onSubmit,
  onRefresh
}: PlayerWalletAdjustModalProps) {
    ...
    try {
      ...
      if (success) {
        ...
        setFormData({
          ...
          amount: 0,
          reason: '',
          version: player.wallet?.version || 0
        });
        setErrors({});
      }
    } finally {
  ...
  const getCurrentValue = (): number => {
    switch (formData.field) {
      case 'balance':
        return player.wallet?.balance || 0;
      case 'frozen_balance':
        return player.wallet?.frozen_balance || 0;
      case 'bonus':
        return player.wallet?.bonus || 0;
      default:
        return 0;
    }
  };
    ...
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle>调整钱包余额 - {player.username}</DialogTitle>
          </DialogHeader>
