      ...
      <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
        ...
        {loading ? (
          ...
          <div className='text-center text-muted-foreground py-8'>
            <p>玩家不存在或加载失败</p>
            <Button variant='outline' className='mt-4' onClick={loadPlayerDetail}>
          ...
          <Tabs defaultValue='basic' className='w-full'>
            ...
            <TabsContent value='basic' className='space-y-4 mt-4'>
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  <label className='text-sm font-medium text-muted-foreground'>玩家ID</label>
                ...
                <div>
                  ...
                  <p className='mt-1'>
                    <Badge variant='default'>{getPlayerStatusText(player.status)}</Badge>
                  </p>
                </div>
                <div>
                  <label className='text-sm font-medium text-muted-foreground'>VIP等级</label>
                  <p className='mt-1'>
                    <Badge variant='outline' className={getVipLevelColor(player.vip_level)}>
                ...
                <div>
                  ...
                  <p className='mt-1'>{getRegistrationMethodText(player.registration_method)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className='mt-1'>{getIdentityCategoryText(player.identity_category)}</p>
                </div>
                <div>
                ...
                <>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <p className='mt-1 font-mono text-lg'>{formatCurrency(player.wallet.balance)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>冻结余额</label>
                      <p className='mt-1 font-mono text-lg'>{formatCurrency(player.wallet.frozen_balance)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>赠送金额</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.bonus)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>积分</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.credit)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>可提现金额</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.withdrawable)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.total_deposit)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>总取款</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.total_withdraw)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>总投注</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.total_bet)}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>总赢取</label>
                      <p className='mt-1 font-mono'>{formatCurrency(player.wallet.total_win)}</p>
                    </div>
                    <div>
              ...
              {player.vip_info ? (
                <div className='grid grid-cols-2 gap-4'>
                  <div>
                    <label className='text-sm font-medium text-muted-foreground'>VIP等级</label>
                    <p className='mt-1'>
                      <Badge variant='outline' className={getVipLevelColor(player.vip_info.level)}>
                  ...
                  <div>
                    ...
                    <p className='mt-1'>{formatDateTime(player.vip_info.last_daily_reward_date)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className='text-sm font-medium text-muted-foreground'>创建时间</label>
                    <p className='mt-1'>{formatDateTime(player.vip_info.created_at)}</p>
                  </div>
                  <div>
                    <label className='text-sm font-medium text-muted-foreground'>更新时间</label>
                    <p className='mt-1'>{formatDateTime(player.vip_info.updated_at)}</p>
                  </div>
                </div>
              ) : (
                <div className='text-center text-muted-foreground py-8'>
                ...
                <>
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <p className='mt-1'>{player.agency.direct_superior_id || '-'}</p>
                    </div>
                    <div>
                      <label className='text-sm font-medium text-muted-foreground'>上级用户名</label>
                      <p className='mt-1'>{player.agency.superior_username || '-'}</p>
                    </div>
                    <div>
                  ...
                  {player.agency.subordinates && player.agency.subordinates.length > 0 && (
                    <div className='mt-4'>
