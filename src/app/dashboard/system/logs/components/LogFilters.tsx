...
interface LogFiltersProps {
  ...
  filters: LogFilters;
  /** 查询回调 */
  onSearch: (filters: Partial<LogFilters>) => void;
  /** 重置回调 */
  onReset: () => void;
  /** 加载状态 */
  loading?: boolean;
...
export function LogFilters({
  ...
  // 本地表单状态
  const [formData, setFormData] = useState<LogFilters>({
    level: 'all',
    module: '',
    action: '',
    search: '',
   ...
   * 更新表单字段值
   */
  const updateFormField = (key: keyof LogFilters, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [key]: value
    }));
