...
import type { DepositOrderFilters } from '../types';
import { DEFAULT_PAGE_SIZE, getDefaultDateRange } from '../constants';

export function useDepositOrderFilters() {
  const defaultDateRange = getDefaultDateRange();
  const [filters, setFilters] = useState<DepositOrderFilters>(() => {
    // 从本地存储恢复筛选条件（可选）
    const storedFilters =
      typeof window !== 'undefined'
        ? localStorage.getItem('depositOrderFilters')
        : null;
  ...
  const searchFilters = useCallback(
    (newFilters: Partial<DepositOrderFilters>) => {
      setFilters((prev) => ({
        ...prev,
        ...newFilters,
        page: 1 // 重置到第一页
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('depositOrderFilters', JSON.stringify(filters));
      } catch (error) {
