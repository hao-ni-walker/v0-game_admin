...
interface OperationReportFiltersProps {
  /** 筛选条件值 */
  filters: OperationReportFilters;
  /** 查询回调 */
  onSearch: (filters: Partial<OperationReportFilters>) => void;
  /** 重置回调 */
  onReset: () => void;
  /** 加载状态 */
  loading?: boolean;
...
export function OperationReportFilters({
  ...
  onReset,
  loading = false
}: OperationReportFiltersProps) {
  // 本地表单状态
  const [formData, setFormData] = useState<OperationReportFilters>({
    dateRange: undefined
  });
   ...
   * 更新表单字段值
   */
  const updateFormField = (key: keyof OperationReportFilters, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [key]: value
    }));
