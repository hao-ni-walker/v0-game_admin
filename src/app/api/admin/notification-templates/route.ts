...
const REMOTE_API_URL =
  'https://api.xreddeercasino.com/api/admin/notification-templates';

/**
 * 获取通知模板列表 API - 代理到远程 API
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!token || !token.value) {
      console.warn('[通知模板] 未授权访问：缺少 token');
    ...
    // 记录请求日志
    console.log('[通知模板] 发送请求到远程API:', remoteUrl);
    ...
    if (result.code !== 200 && result.code !== 0) {
      console.warn('[通知模板] 远程API返回错误:', {
        code: result.code,
        msg: result.msg
      });
      return errorResponse(result.msg || '获取通知模板列表失败');
    }
    ...
    const requestDuration = Date.now() - requestStartTime;
    console.error('[通知模板] 获取模板列表失败:', {
      error: error instanceof Error ? error.message : String(error),
      requestDuration: `${requestDuration}ms`
    });

    return errorResponse('获取通知模板列表失败');
  }
}
