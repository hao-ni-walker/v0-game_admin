...
async function forward(
  ...
  const remoteResponse = await requestRemoteAdminApi<{
    ...
    path: `/api/v1/admin/automation/tasks/${id}`,
    method,
    headers: {
  ...
  if (!remoteResponse.ok) {
    ...
    if (remoteResponse.status === 400 || remoteResponse.status === 409) {
      return errorResponse(remoteResponse.data?.message || '操作失败');
    }
...
export async function PUT(
  ...
  try {
    ...
    return errorResponse('更新自动化任务失败');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return errorResponse('删除自动化任务失败');
  }
}
