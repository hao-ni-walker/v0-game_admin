...
const LIST_PATH = '/api/v1/admin/prediction/markets';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '获取预测市场列表失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('获取预测市场列表失败:', error);
    return errorResponse('获取预测市场列表失败');
  }
}
