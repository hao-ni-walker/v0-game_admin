...
const LIST_PATH = '/api/v1/admin/currencies';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '获取币种列表失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('获取币种列表失败:', error);
    return errorResponse('获取币种列表失败');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = remoteResponse.data;
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '创建币种失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('创建币种失败:', error);
    return errorResponse('创建币种失败');
  }
}
