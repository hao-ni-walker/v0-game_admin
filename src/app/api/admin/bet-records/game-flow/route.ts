...
const REMOTE_API_URL =
  'https://api.xreddeercasino.com/api/admin/bet-records/game-flow';

/**
 * 获取游戏流水列表 API - 代理到远程 API
 * GET /api/admin/bet-records/game-flow
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!token || !token.value) {
      console.warn('[游戏流水] 未授权访问：缺少 token');
    ...
    // 记录请求日志
    console.log('[游戏流水] 发送请求到远程API:', remoteUrl);
    ...
    if (result.code !== 200 && result.code !== 0) {
      console.warn('[游戏流水] 远程API返回错误:', {
        code: result.code,
        msg: result.msg
      });
      return errorResponse(result.msg || '获取游戏流水失败');
    }
    ...
    const requestDuration = Date.now() - requestStartTime;
    console.error('[游戏流水] 获取游戏流水失败:', {
      error: error instanceof Error ? error.message : String(error),
      requestDuration: `${requestDuration}ms`
    });

    return errorResponse('获取游戏流水失败');
  }
}
