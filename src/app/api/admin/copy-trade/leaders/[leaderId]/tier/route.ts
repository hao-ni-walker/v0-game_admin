...
export async function POST(
  ...
  try {
    ...
    const remoteResponse = await requestRemoteAdminApi<{ code?: number; message?: string; data?: unknown }>({
      path: `/api/v1/admin/copy-trade/leaders/${leaderId}/tier`,
      method: 'POST',
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '调整等级失败');
    }
    return successResponse(result.data);
  } catch (error) {
    console.error('调整带单员等级失败:', error);
    return errorResponse('调整带单员等级失败');
  }
}
