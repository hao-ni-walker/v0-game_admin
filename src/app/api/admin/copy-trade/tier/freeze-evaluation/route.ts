...
export async function POST(req: NextRequest) {
  try {
    ...
    const remoteResponse = await requestRemoteAdminApi<{ code?: number; message?: string; data?: unknown }>({
      path: '/api/v1/admin/copy-trade/tier/freeze-evaluation',
      method: 'POST',
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '冻结评估失败');
    }
    return successResponse(result.data);
  } catch (error) {
    console.error('冻结等级评估失败:', error);
    return errorResponse('冻结等级评估失败');
  }
}
