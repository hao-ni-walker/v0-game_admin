...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!token || !token.value) {
      console.warn('[活动管理] 未授权访问：缺少 token');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[活动管理] 创建活动请求:', JSON.stringify(body, null, 2));
    ...
    console.log('[活动管理] 创建活动响应:', JSON.stringify(result, null, 2));
    ...
    return errorResponse(result.msg || '创建活动失败');
  } catch (error) {
    console.error('[活动管理] 创建活动失败:', error);
