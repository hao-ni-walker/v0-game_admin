...
const MANUAL_PATH = '/api/v1/admin/settlement/manual-trigger';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '手动触发结算失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('手动触发结算失败:', error);
    return errorResponse('手动触发结算失败');
  }
}
