...
export async function POST(
  ...
  try {
    ...
    const remoteResponse = await requestRemoteAdminApi<{
      ...
      path: `/api/v1/admin/settlement/exceptions/${id}/resolve`,
      method: 'POST',
      headers: {
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '处理异常结算失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('处理异常结算失败:', error);
    return errorResponse('处理异常结算失败');
  }
}
