...
const FAILURES_PATH = '/api/v1/admin/trade/failures';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!remoteResponse.ok) {
      ...
      if (remoteResponse.status === 404) return errorResponse('远程接口未部署: /api/v1/admin/trade/failures');
      return errorResponse(`远程API错误: ${remoteResponse.status}`);
    }

    const result = remoteResponse.data;
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '获取交易失败记录失败');
    }

    return successResponse(result.data);
  } catch (error) {
    console.error('获取交易失败记录失败:', error);
    return errorResponse('获取交易失败记录失败');
  }
}
