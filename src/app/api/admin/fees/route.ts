...
const LIST_PATH = '/api/v1/admin/fees';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '获取费率配置失败');
    }
    return successResponse(result.data);
  } catch (error) {
    console.error('获取费率配置失败:', error);
    return errorResponse('获取费率配置失败');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = remoteResponse.data;
    if (!result || (result.code !== 0 && result.code !== 200)) {
      return errorResponse(result?.message || '保存费率配置失败');
    }
    return successResponse(result.data);
  } catch (error) {
    console.error('保存费率配置失败:', error);
    return errorResponse('保存费率配置失败');
  }
}
