...
export async function PUT(
  ...
  try {
    ...
    if (!token || !token.value) {
      await logger.warn('玩家管理', '更新玩家状态', '未授权访问：缺少 token', {
        playerId: id,
    ...
    if (!remoteResponse.ok) {
      ...
      await logger.error('玩家管理', '更新玩家状态', '远程API请求失败', {
        playerId: id,
    ...
    // 记录成功日志
    await logger.info('玩家管理', '更新玩家状态', '更新玩家状态成功', {
      playerId: id,
    ...
    // 如果远程 API 返回错误
    if (result.code !== 200 && result.code !== 0) {
      await logger.warn('玩家管理', '更新玩家状态', '远程API返回错误', {
        playerId: id,
        code: result.code,
        msg: result.msg,
        timestamp: new Date().toISOString()
      });
      return errorResponse(result.msg || '更新玩家状态失败');
    }

    return successResponse(result.data || result);
  } catch (error) {
    await logger.error('玩家管理', '更新玩家状态', '更新玩家状态失败', {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
      timestamp: new Date().toISOString()
    });

    return errorResponse('更新玩家状态失败');
  }
}
