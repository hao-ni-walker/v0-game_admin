...
const REMOTE_API_URL = 'https://api.xreddeercasino.com/api/admin/users/statistics';

/**
 * 玩家统计 API - 代理到远程 API
 * POST /api/players/statistics
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token || !token.value) {
      await logger.warn('玩家管理', '获取玩家统计', '未授权访问：缺少 token', {
    ...
    if (!remoteResponse.ok) {
      ...
      await logger.error('玩家管理', '获取玩家统计', '远程API请求失败', {
    ...
    // 记录成功日志
    await logger.info('玩家管理', '获取玩家统计', '获取玩家统计成功', {
      timestamp: new Date().toISOString()
    });
    ...
    if (result.code !== 200 && result.code !== 0) {
      await logger.warn('玩家管理', '获取玩家统计', '远程API返回错误', {
        code: result.code,
        msg: result.msg,
        timestamp: new Date().toISOString()
      });
      return errorResponse(result.msg || '获取玩家统计失败');
    }

    return successResponse(result.data || result);
  } catch (error) {
    await logger.error('玩家管理', '获取玩家统计', '获取玩家统计失败', {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
      timestamp: new Date().toISOString()
    });

    return errorResponse('获取玩家统计失败');
  }
}
